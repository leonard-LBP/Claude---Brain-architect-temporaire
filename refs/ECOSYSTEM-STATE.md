...
**Phase A3 close (toutes sous-phases incluant A3.6.fix).** Cumul A1+A2+A3 : **301 actions Notion** (43 manuels + 20 templates + 96 Glossaire + 62 Registre + 6 repairs collisions + 73 codes migrés `GLO_*` + 1 DDL Glossaire). 72/72 concepts couverts. **A3.6.fix** : 8 codes Glossaire ratés détectés par Leonard via spot-check manuel (3 anciens format `CPT.X.LBP.Y` jamais matched + 5 sub-phase A3.bis.e ajoutées inline sans capture dans `a3bis_plan.json` → invisible pour le script de migration). Tous corrigés. Architecture Glossaire / Registre stabilisée : Glossaire = identité + sémantique du concept (Code, Aliases, Mots clés, Type, Définition, Valeur ajoutée, Règles d'usage, Usages IA, Équivalent langage courant) ; Registre = méta du fichier source (Code `CPT_*`, Version du template, Statut, Lien Drive). Captures règles : C-013, C-014, C-015 + amendement R-054 (paire `CPT/GLO`). Outils : `scripts/lib/notion_keys.py`, `scripts/phase_a3/build_drive_url_resolver.py`, `scripts/phase_a3/a3bis_plan.py`, `scripts/phase_a3/a3_6_migration.py`. Prochaine étape : **Phase A4** (Taxonomies, 102 taxos) + relations utilise (taxonomies) cross-BDDs.
