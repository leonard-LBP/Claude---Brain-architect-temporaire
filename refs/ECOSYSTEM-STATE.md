...
> Dernière mise à jour : 29-04-2026 — **Phase A4.B close.** Mass apply `utilise (taxonomies)` sur les 42 manuels de BDD (Brain 11 + Twin 26 + MO 4, hors `Pratiques organisationnelles` Twin sans section Usages des taxonomies dans le .md) : 42 update_page Notion, ~279 relations posées, 0 erreur. Doctrine adoptée : codes extraits de la section « Usages des taxonomies » (§5 Brain, §6 Twin/MO) du .md → résolus via `taxo_code_to_url.json` (102 codes mappés). Vérification 4/4 conforme (Prompts LBP, Enjeux, Sources d'informations, Processus candidats sandbox).

## Phase actuelle

**Phase A4.A + A4.B close.** Cumul A1+A2+A3+A4.A+A4.B : **446 actions Notion** (302 A3 + 99 A4.A sync taxos + 3 A4.A calibration + 42 A4.B). **Phase A4.B** (relations Manuels ↔ Taxonomies) : 42 manuels mis à jour avec `utilise (taxonomies)` peuplé depuis le parsing de leur section « Usages des taxonomies ». 87 codes uniques référencés transverse, 100% mappables. Anomalie détectée : `Manuel de BDD - Pratiques organisationnelles` (Twin) s'arrête à §4 — pas de section taxos (à compléter ultérieurement). **Phase A4.A** (Taxonomies) : 102 taxos Markdown synchronisées avec BDD `Registre des taxonomies` Notion. 96 updates (codes `<X>.<Y>.LBP` migrés vers `<X>.<Y>` per R-054, props complètes : Aliases, Descriptions, Lien Drive, Nature sémantique, Mode de sélection, Ouverture, Statut Validé, Version 2.0) + 6 creates (BRAIN.LAYER, BRAIN.SUBTYPE, DBMAN.SCOPE, DOC.TYPE, JOB.COVERAGE, LGBLK.FAMILY). Architecture Glossaire / Registre stabilisée : Glossaire = identité + sémantique du concept (Code, Aliases, Mots clés, Type, Définition, Valeur ajoutée, Règles d'usage, Usages IA, Équivalent langage courant) ; Registre = méta du fichier source (Code `CPT_*`, Version du template, Statut, Lien Drive). Captures règles : C-013, C-014, C-015 + amendement R-054 (paire `CPT/GLO`). Outils : `scripts/lib/notion_keys.py`, `scripts/phase_a3/build_drive_url_resolver.py`, `scripts/phase_a3/a3bis_plan.py`, `scripts/phase_a3/a3_6_migration.py`. Prochaine étape : **Phase A4** (Taxonomies, 102 taxos) + relations utilise (taxonomies) cross-BDDs.
