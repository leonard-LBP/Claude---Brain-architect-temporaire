...
**Phase A3 close DÉFINITIF (toutes sous-phases + A3.7 + cleanup I/O Processus).** Cumul A1+A2+A3 : **302 actions Notion** + cleanup manuel Leonard (renommage Registre `Note de Concept - X` → `Concept - X`, archive paire I/O Processus 22-fév). **A3.7** : convention naming `Concept - X` formalisée dans manuel + WR-RD du Registre (instruction d'écriture du champ Nom canonique). **Audit final relations Registre↔Glossaire** : 72/72 concepts Markdown bien reliés bidir (post-archive Leonard). Architecture stabilisée : Glossaire = `GLO_*` (sémantique du concept) + Registre = `CPT_*` (méta du fichier source) ; paire `CPT_X_Y ↔ GLO_X_Y` 1:1 pour chaque concept Drive ; Version du template natif Registre + rollup Glossaire ; Statut natif chacun (avec fan-out sémantique). Captures règles : C-013 (push systématique), C-014 (apostrophes ASCII MCP), C-015 (vocabulaire « notes de concept »), R-054 amendé (paire CPT/GLO). Architecture Glossaire / Registre stabilisée : Glossaire = identité + sémantique du concept (Code, Aliases, Mots clés, Type, Définition, Valeur ajoutée, Règles d'usage, Usages IA, Équivalent langage courant) ; Registre = méta du fichier source (Code `CPT_*`, Version du template, Statut, Lien Drive). Captures règles : C-013, C-014, C-015 + amendement R-054 (paire `CPT/GLO`). Outils : `scripts/lib/notion_keys.py`, `scripts/phase_a3/build_drive_url_resolver.py`, `scripts/phase_a3/a3bis_plan.py`, `scripts/phase_a3/a3_6_migration.py`. Prochaine étape : **Phase A4** (Taxonomies, 102 taxos) + relations utilise (taxonomies) cross-BDDs.
