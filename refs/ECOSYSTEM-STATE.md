...
> Dernière mise à jour : 29-04-2026 — **Phase A3 + A3.bis + A3.ter + A3.quater + A3.5 closes.** **72/72 concepts couverts** via 164 updates Notion (96 Glossaire + 62 Registre + 6 repairs collisions). Phase A3.5 a complété les 4 rich-text restants (Notes, Règles d'usage et pièges, Usages IA potentiels, Équivalent langage courant) sur les 4 entrées 25-avr (Collectif, Poste, Initiative organisationnelle, Relation inter-organisations) — toutes rédactions sous 500 chars par propriété. Reste : 1 doublon I/O Processus à archiver manuellement par Leonard. Question archi en suspens : Code unique / Version du template — natif côté Glossaire ou rollup depuis Registre ? (à arbitrer avant Phase A4).

## Phase actuelle

**Phase A3 + A3.bis + A3.ter + A3.quater + A3.5 closes.** Cumul A1+A2+A3 : **227 entrées Notion mises à jour structurellement** (43 manuels + 20 templates + 96 Glossaire + 62 Registre + 6 repairs). 72/72 concepts couverts. Captures règles : C-013 (push systématique), C-014 (apostrophes ASCII MCP Notion), C-015 (vocabulaire « notes de concept » vs « notes Drive »). Outils créés : `scripts/lib/notion_keys.py` (helper apostrophes), `scripts/phase_a3/build_drive_url_resolver.py` (DriveFS sqlite resolver), `scripts/phase_a3/a3bis_plan.py` (orchestration). Prochaine étape : **arbitrage archi Glossaire** (Code unique / Version du template natif vs rollup) puis **Phase A4** (Taxonomies, 102 taxos) + relations utilise (taxonomies) cross-BDDs.
