...
- [28-04-2026] **Chantier M — Architecture de la connaissance méta : maintenabilité, évolutivité, propagation, anti-doublons**
  - **Contexte enrichi (Leonard)** : la connaissance méta doit être **maintenable** (éviter asymétries entre docs, évolution coordonnée), **évolutive** (capable de croître proprement avec l'écosystème), **sans doublons** (un sujet = un endroit), et **accessible contextuellement par les agents** (clair, simple, optimal). Les docs méta seront sujets à **logiques de propagation** entre eux (analogue à R-041/R-042 manuel↔WR-RD mais transverse aux docs méta).
  - **Question d'architecture (à arbitrer en Phase D1)** : faut-il une **charte globale LBP** + des **mini-chartes spécifiques** rattachées (ex : charte graphique 5D, charte visuelle cartes de causalité des problématiques, etc.) ? Le style visuel doit rester harmonisé donc dicté par une charte graphique globale, mais les déclinaisons spécifiques ont besoin de leurs propres règles. Pattern : doc parent (charte globale) ↔ docs enfants (mini-chartes par domaine), avec règles d'héritage et d'override claires.
  - **Mêmes questions** pour les playbooks techniques, workflows, doctrines : un seul gros doc ou bundle de mini-docs articulés ?
  - **Capital insight** : tout au long de la session 27-28-04-2026, on a accumulé de **nombreuses règles, décisions, patterns et apprentissages** (R-053→R-059, D-019/020/021, C-009/010/011, patterns DDL Notion, conventions de nommage, doctrine d'isolation Brain↔MO/Twin, logique de propagation Manuel↔WR-RD, etc.). Ces apprentissages sont la matière première du futur bundle docs méta — on a déjà une bonne partie du contenu, reste à l'organiser proprement par périmètre.

  → **Cette entrée est un dépassement de la simple réactualisation du PLAYBOOK** : c'est une refonte architecturale du bundle docs méta. À aborder en Phase D avec une vraie réflexion en amont (D1).

- [28-04-2026] **Phase E — Pré-requis : "bouquiner" tous les contenus du dossier Prompts**
  - **Contexte (Leonard)** : au début de la Phase E, je devrai lire tous les contenus du dossier `Architecture data/Prompts/`. Même si ces infos sont désuètes (extraites du code de l'app il y a ~1 mois et ayant évolué depuis), ça me donnera une **couche de compréhension supplémentaire** sur ce qui se passe dans l'app LBP.
  - **Utilité** : repenser les system prompts / prompts maîtres / logic blocks autour de l'agent **Twin architect**, puis faire de même pour l'agent **Brain architect** (qui n'a actuellement qu'une ébauche de system prompt, pas de prompts maîtres ni logic blocks pour les opérations sur le Brain).
  - **Insight** : les opérations nombreuses et structurantes menées dans la session 28-04-2026 (migration au canon, audit Notion, sync DDL, capture règles/décisions, refonte templates, etc.) sont de **formidables insights** pour alimenter Brain architect en instructions claires :
    - dans son system prompt (règles transverses : R-053→R-059, doctrines, isolation, hygiène d'écriture)
    - dans ses prompts maîtres (opérations standardisées : migration au canon, audit symétrie Manuel↔Notion, sync DDL, propagation R-041/R-042)
    - dans ses logic blocks (adaptés aux opérations × BDDs Brain : Extract, Refactor, Dédoublonnage, Relations Maker, etc., adaptés aux objets Brain)
  - **À noter pendant Phase A et Phase B** : capturer en continu tout pattern/règle/décision intéressant qui pourrait nourrir le system prompt et les prompts maîtres de Brain architect.
