...
## 1.6 Traçabilité d'exécution : responsabilité orthogonale

La traçabilité opérationnelle du déclenchement d'un WF (date, contexte, résultat, anomalies rencontrées) est une **responsabilité orthogonale** au catalogue : elle est gérée par le journal de session de l'exécutant (par exemple `ECOSYSTEM-STATE.md` côté collaboration Claude/Leonard, journaux dédiés côté Brain Architect / Twin Architect / Mission Architect, logs côté scripts automatisés). Les items WF du catalogue ne mentionnent donc pas item par item le mécanisme de traçabilité de l'exécutant — par symétrie avec §1.4 (agent-agnosticité). Un WF orchestrateur peut cependant inclure une étape « tracer dans le journal de session de l'exécutant » quand la traçabilité est intrinsèque au sens du WF (ex. workflow de propagation transverse, voir [[#WF-008]]).
...
> **Note** : l'ordre des items au sein de chaque sous-section §5.x suit le **flux d'usage typique** (ce qu'on enchaîne dans la pratique), pas l'ordre chronologique d'apparition des IDs. Un agent qui consulte une sous-section voit ainsi les WF dans l'ordre où il va les composer.
...
| WF-023 | Capturer un nouveau workflow opérationnel WF-XXX | 5.2 Cycle de vie d'un catalogue doc méta | 03-05-2026 |
...
- **Étapes** :
  ...
  5. Appliquer la QA stricte d'égalité ([[Règles de propagation - LBP#PROP-001]]) : vérifier cellule par cellule l'égalité mot pour mot entre le WR-RD et la section 4 du manuel parent sur les 9 colonnes retenues. Tolérances admises : adaptations typographiques liées au rendu Markdown.
  ...
  7. **Cas re-génération** (le WR-RD existait déjà) : bumper `version` du WR-RD selon [[Règles intrinsèques - LBP#R-056]] (MINOR si modifications de cellules ; MAJOR si refonte structurelle de sections). Mettre à jour `updated_at`. Si le template a évolué entre-temps, mettre à jour aussi `template_version`.
  8. Récupérer l'URL Drive du WR-RD via [[#WF-011]] et la coller dans la propriété `Lien vers le doc WR-RD (.md)` de la fiche Notion correspondant au manuel parent dans la BDD `Manuels de BDD`.
  9. Vérifier la conformité finale : le WR-RD est-il bien dans `Manuels de BDD/{Domain}/WR-RD/` ? Frontmatter complet et conforme (version, template_version, updated_at à jour) ? Sections (ou subset) correctes ? Fiche Notion du manuel parent pointe-t-elle vers le WR-RD ?
- **Sortie** :
...
- **Étapes** :
  ...
  10. Vérifier que les entrées sont bien créées et reliées (relations bidirectionnelles effectives, pas de dangling).
- **Sortie** :
...
- **Articulation** : [[Règles intrinsèques - LBP#R-001]] (Markdown SoT), [[Règles intrinsèques - LBP#R-029]] (Markdown SoT pour indexation Notion), [[Règles intrinsèques - LBP#R-030]] (double indexation note de concept), [[Règles intrinsèques - LBP#R-031]] (alignement code CPT/GLO), [[Règles intrinsèques - LBP#R-032]] (mise à jour plutôt que création), [[Règles intrinsèques - LBP#R-033]] (descriptions Notion = mini-prompts), [[Règles intrinsèques - LBP#R-034]] (ordonnancement création puis relation), [[Règles intrinsèques - LBP#R-036]] (code unique = identité), [[Règles intrinsèques - LBP#R-038]] (identifiant pivot par type), [[Règles intrinsèques - LBP#R-039]] (anti-artefacts), [[Règles intrinsèques - LBP#R-069]] (lecture complète), [[Règles intrinsèques - LBP#R-072]] (anti-énumération taxons), [[#WF-011]] (URL Drive), [[#WF-016]] (audit post-indexation), [[#WF-008]] (cascade plus large si modification du doc source).
...
- **Garde-fous & anti-patterns** :
  ...
  - ❌ Énumérer en `Conséquences` toutes les dérivées hypothétiques sans les capturer ensuite — soit on capture les R/PROP/WF dérivées dans la même unité de travail, soit on les liste comme TODO dans le journal de session de l'exécutant (cf. §1.6) pour suivi.
...
## WF-023 : Capturer un nouveau workflow opérationnel WF-XXX

- **Portée** : Transverse
- **Trigger** : Observation d'une procédure opérationnelle répétitive standardisable, ou besoin émergent d'orchestration entre plusieurs cascades / opérations atomiques. Cas typiques : une procédure ad hoc a été exécutée manuellement 2-3 fois avec succès et mérite codification ; un nouveau type d'opération apparaît avec l'évolution de l'écosystème ; une méthode de niveau supérieur (future BDD `Méthodes LBP`) en cours d'élaboration révèle une brique opérationnelle non cataloguée.
- **Préconditions** :
  - La procédure est descriptible en pas-à-pas avec un trigger nommable, des préconditions identifiables, des étapes ordonnées, une sortie attendue (cf. §1.3 granularité d'item).
  - La procédure n'attribue pas à un agent spécifique (cf. §1.4 agent-agnosticité). Si l'attribution est intrinsèque à l'opération, c'est une **méthode** (future `Méthodes LBP`), pas un WF.
  - Toutes les contraintes opposables que la procédure mobilise sont déjà capturées en R-XXX / PROP-XXX / D-XXX, ou identifiées comme manquantes à capturer (cf. §3.1 garde-fou — un WF applique les R, ne les redéfinit pas).
- **Étapes** :
  1. Décrire la procédure : nommer le **Trigger** (1 phrase), les **Préconditions** (état du système requis), les **Étapes** ordonnées (5-10 typiquement), la **Sortie** attendue (livrables concrets).
  2. **Inventorier les R / PROP / D / autres WF mobilisés** par la procédure. Pour chaque contrainte, vérifier qu'elle existe dans le catalogue cible. Si une contrainte n'est pas catalogable, **déclencher [[#WF-019]]** en parallèle pour la capturer en R / PROP / D / code / C, **avant** de finaliser le WF (sinon le WF reposerait sur des contraintes implicites — viole §3.1).
  3. Lister les **Garde-fous & anti-patterns** connus (optionnel) : pièges observés lors des exécutions ad hoc, comportements à proscrire, asymétries silencieuses possibles.
  4. Construire l'**Articulation** : wikilinks denses vers R / PROP / D / autres WF cités à l'étape 2, avec une parenthèse explicative par lien (rôle du lien dans la procédure).
  5. Choisir la sous-section thématique de [[#5) Catalogue]] (5.1 Cycle de vie d'une BDD / 5.2 Cycle de vie d'un catalogue doc méta / 5.3 Audits & migrations transverses / 5.4 Cascades, utilitaires & sync transverses / future 5.5 Production des autres docs canoniques cf. §1.5).
  6. Si aucune sous-section existante ne convient et qu'on touche à une famille structurellement nouvelle (ex. premier WF de production de doc canonique non-BDD non-catalogue) : créer §5.5 anticipée par bump MINOR du catalogue (cf. §1.5 et [[Règles intrinsèques - LBP#R-063]]).
  7. Proposer ID + Nom (verbe d'action en tête, 4-12 mots) + sous-section pour validation explicite (cf. protocole de capture proactive).
  8. Si validé : ajouter le WF dans le bon §5.x du catalogue + mettre à jour le récap §4.
  9. Bumper la `version` du catalogue selon [[Règles intrinsèques - LBP#R-063]] (PATCH pour ajout d'entrée atomique, MINOR si création d'une nouvelle sous-section).
  10. Mettre à jour la fiche Notion `Workflows opérationnels - LBP` dans BDD `Docs méta LBP` (`Version du document`, `updated_at`).
  11. Appliquer [[#WF-021]] (audit grep post-instanciation) sur le catalogue mis à jour pour vérifier conformité (wikilinks intra-doc, champ Origine rempli, pas de marqueur temporel résiduel, YAML parseable, parité §4 vs §5).
  12. Mirror `refs/` du repo collab + commit + push sur les 2 repos pertinents.
- **Sortie** :
  - 1 nouveau WF-XXX capturé dans le catalogue avec ID immuable.
  - Catalogue [[Workflows opérationnels - LBP]] bumpé + fiche Notion alignée.
  - Audit grep [[#WF-021]] passant.
  - Si applicable : R / PROP / D dérivées capturées en parallèle via [[#WF-019]].
  - Mirror `refs/` à jour, commit + push effectués.
- **Garde-fous & anti-patterns** :
  - ❌ Capturer une procédure trop fine qui ne mérite pas son propre WF (test : passe-t-elle le critère §1.3 « non décomposable sans perte de sens » ?). Symptôme : étapes triviales, pas de garde-fous réels.
  - ❌ Capturer une procédure trop large qui devrait être éclatée en N WF distincts (test : 1 trigger + 1 sortie unitaires ?). Symptôme : étapes qui appellent N autres WF sans logique propre — c'est un fourre-tout (cf. §1.3 anti-pattern).
  - ❌ Capturer un WF qui attribue à un agent (« Claude exécute X », « le brain architect lance Y »). Réécrire en agent-agnostique (« exécuter X », « lancer Y ») — sinon c'est une méthode, pas un WF (cf. §1.4 et §3.4).
  - ❌ Définir des contraintes opérationnelles dans les étapes du WF qui ne sont citées nulle part en R / PROP / D — viole §3.1 (un WF applique les R, ne les redéfinit pas). Soit la contrainte est triviale et reste dans l'étape, soit elle mérite capture en R-XXX (déclencher [[#WF-019]]).
  - ❌ Ne pas inventorier les wikilinks vers R / PROP / D en `Articulation` — un WF sans articulation est un WF orphelin, non-traçable.
- **Articulation** : [[Règles intrinsèques - LBP#R-063]] (politique bump), [[Règles intrinsèques - LBP#R-066]] (propriétaire canonique unique), [[Règles intrinsèques - LBP#R-070]] (Brain agent-agnostique), [[Règles intrinsèques - LBP#R-074]] (méthodes pour règles de maintenance — un WF ≠ une méthode), [[Règles de propagation - LBP#PROP-006]] (cascade doc méta indexé), [[#WF-019]] (capture R/PROP/D dérivée éventuellement), [[#WF-021]] (audit grep post-instanciation), [[#WF-022]] (production D-XXX, schéma analogue mais pour décisions).
- **Origine** : 03-05-2026, identifié comme lacune lors de l'audit du catalogue Workflows opérationnels - LBP v1.0 (Leonard a flaggé l'asymétrie : WF-019 capture R/PROP/D/code/C mais pas WF-XXX, alors que WF a un schéma propre plus structuré qui mérite son workflow dédié).

## WF-021 : Auditer par grep un doc catalogue après instanciation

- **Portée** : Transverse (tous catalogues docs méta instanciés depuis [[Template - META Catalogue]] ou un futur template de catalogue)
...
- **Étapes** :
  1. **Wikilinks intra-doc + exclusion des blocs code** : tout renvoi à un autre item du même catalogue est-il en wikilink interne `[[#PREFIXE-XXX]]` (et non en prose brute « voir R-027 ») ? Script Python avec regex appropriée à la profondeur de heading du catalogue : `^#### ` (avec espace) pour H4 strict (catalogues R/PROP/D), `^## ` (avec espace) pour H2 strict (catalogue WF). Sans l'espace, on capte aussi les niveaux supérieurs et le body extrait est tronqué (bug détecté Phase 4.1). **Exclure aussi les blocs code** entre triples backticks : le mini-exemple §2.3 reproduit un item à des fins d'illustration, et un grep naïf le compte 2x — seule l'occurrence en §5.x est canonique. Filtrer les régions entre triples backticks avant de compter ou d'extraire les items.
...
- **Étapes** :
  ...
  5. Lister exhaustivement les instances stale (artefact intermédiaire : tableau ou liste réutilisable pour planifier le chantier de migration ; un chantier MAJOR n'est pas un commit isolé).
...
- **Garde-fous & anti-patterns** :
  ...
  - ❌ Migrer les instances en silence sans inventaire préalable → perte de traçabilité incrémentale.
...
- **Étapes** :
  ...
  3. **Phase 3 — Propager dans l'ordre strict des dérivés**. Ne jamais sauter d'étape, ne jamais réordonner : (a) **Markdown d'abord** : le doc dérivé Markdown est mis à jour ; (b) **Versions bumpées** : si le contenu structurant change ([[Règles intrinsèques - LBP#R-056]] et [[Règles intrinsèques - LBP#R-063]]), bumper la version + `updated_at` ; (c) **Notion ensuite** : une fois le Markdown stable, aligner Notion (DDL si schéma, propriétés textuelles si contenu) ; (d) **Renvois croisés** : si le doc modifié est cité ailleurs (autres docs Markdown, journal de session, mirrors), grep + remplacement systématique.
  ...
  6. **Phase 6 — Tracer dans le journal de session de l'exécutant** (cf. §1.6). Mise à jour systématique du journal vivant : date de la propagation, doc source, dérivés alignés, anomalies. Pas de batch — une trace par phase de propagation pour préserver la traçabilité incrémentale.
  ...
  **Tableau des dérivés directs par type de doc source** :
  ...
  | **Règle R-XXX** (création/modification) | [[Règles intrinsèques - LBP]] + journal de session + docs concernés si format-impactant | [[Règles de propagation - LBP#PROP-007]] |
  | **Décision D-XXX** | [[Décisions architecturales - LBP]] + R/PROP/WF dérivées + journal de session | [[Règles de propagation - LBP#PROP-008]] |
  | **Convention C-XXX** | `CLAUDE.md` + journal de session | [[Règles de propagation - LBP#PROP-009]] |
...
- **Sortie** :
  ...
  - Trace dans le journal de session de l'exécutant (cf. §1.6).
...
- **Garde-fous & anti-patterns** :
  ...
  - ❌ Sauter la phase de traçabilité dans le journal de session (perte de traçabilité incrémentale).
  ...
  - ❌ Batcher plusieurs phases en une seule trace journal — une trace par phase de propagation.
